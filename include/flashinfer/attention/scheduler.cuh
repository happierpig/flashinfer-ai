...
struct HolisticPlanInfo {
  ...
  static constexpr uint32_t NUM_TASK_ARGS = 12;
  ...
  void FromVector(const std::vector<int64_t>& vec) {
    ...
    for (uint32_t i = 0; i < NUM_TASKS; ++i) {
      ...
      tasks[i].barrier_idx_offset = vec[2 + i * NUM_TASK_ARGS + 11];
    }
    ...
    merged_barrier_idx_offset = vec[6 + NUM_TASKS * NUM_TASK_ARGS];
    num_qo_len_offset = vec[7 + NUM_TASKS * NUM_TASK_ARGS];
    barrier_vec_offset = vec[8 + NUM_TASKS * NUM_TASK_ARGS];
  }
};
...
inline cudaError_t TwoStageHolisticPlan(void* float_buffer, size_t float_workspace_size_in_bytes,
  ...
  std::vector<uint32_t> barrier_vec;
  ...
  std::vector<uint32_t> merged_barrier_idx;
  ...
  for (uint32_t task = 0; task < NUM_TASKS; ++task) {
    ...
    if (total_num_works > max_total_num_works) {
      std::ostringstream err_msg;
      err_msg << "total_num_works " << total_num_works << " exceeds max_total_num_works "
              << max_total_num_works;
      FLASHINFER_ERROR(err_msg.str());
    }
  ...
  plan_info.merged_barrier_idx_offset = int_allocator.aligned_alloc_offset(
      sizeof(uint32_t) * max_packed_qo_lens, 16, "merged_barrier_idx");
