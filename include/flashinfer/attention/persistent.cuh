...
#pragma unroll
      ...
      if (num_kv_chunks > 1) {
        ...
        // write through
        // NOTE (Yilong): assume o_stride_n = num_qo_heads* head_dim
        ...
        write_o_reg_gmem<KTraits>(o_frag, &q_smem, o_ptr_base, qo_packed_idx_base, q_len,
                                  ...
                                  /*o_stride_h=*/o_stride_h, gqa_group_size, tid);
      }
...
__device__ __forceinline__ void logits_to_scores_and_reduction(
...
#pragma unroll
      for (uint32_t reg_id = 0; reg_id < 8; ++reg_id) {
        // scale logits into scores
        s_frag[mma_q][mma_kv][reg_id] = math::ptx_exp2(
            s_frag[mma_q][mma_kv][reg_id] * variant.sm_scale_log2 - lse[mma_q][(reg_id % 4) / 2]);
      }
    }
  }

  // reduce along qo dimension into s_frag[0][mma_kv][0,1,4,5]
#pragma unroll
  for (uint32_t mma_kv = 0; mma_kv < KTraits::NUM_MMA_KV; ++mma_kv) {
#pragma unroll
    for (uint32_t reg_id = 0; reg_id < 4; ++reg_id) {
      // reduce thread-local scores
      float thr_local_sum = 0.;
#pragma unroll
      for (uint32_t mma_q = 0; mma_q < KTraits::NUM_MMA_Q; ++mma_q) {
#pragma unroll
        for (uint32_t j = 0; j < 2; ++j) {
          const uint32_t q_idx = q[mma_q][j];
          thr_local_sum +=
              (q_idx < qo_len) ? s_frag[mma_q][mma_kv][(reg_id / 2) * 4 + reg_id % 2 + j * 2] : 0.;
        }
      }
      // reduce warp-local scores
      // group by (lane_idx % 4); reduction among sub-group (size=8)
#pragma unroll
...
#pragma unroll
  ...
  // call LogitsTransform to dump scores
  // only first warp && first `num_n_lanes` lanes && 4 n registers are called
  if (warp_idx_q == 0 && lane_idx < num_n_lanes) {
    constexpr uint32_t mma_q = 0;
#pragma unroll
    for (uint32_t mma_kv = 0; mma_kv < KTraits::NUM_MMA_KV; ++mma_kv) {
#pragma unroll
      for (uint32_t reg_id = 0; reg_id < 4; ++reg_id) {
        const uint32_t q_idx = q[mma_q][0], kv_idx = kv_idx_base + mma_kv * 16 +
                                                     2 * (lane_idx % 4) + 8 * (reg_id / 2) +
                                                     reg_id % 2;
        const uint32_t qo_head_idx = kv_head_idx * group_size + r[mma_q][0];
        variant.LogitsTransform(params, s_frag[0][mma_kv][reg_id], batch_idx, q_idx, kv_idx,
...
template <typename KTraits_, typename Params_>
struct BlockBatchAttentionScorePersistent {
  using KTraits = KTraits_;
  using Params = Params_;
  ...
  static __device__ __forceinline__ void Run(const Params& params,
    ...
    // this is only relate to qo_idx, qo_head_idx, thus can initilized once
    float lse[NUM_MMA_Q][2];
    float s_frag[NUM_MMA_Q][NUM_MMA_KV][8];
    ...
    // use vsmem for CTA-level scores reduction
    float* scores_smem = reinterpret_cast<float*>(smem_storage->v_smem);
    smem_t<SWIZZLE_MODE_Q> q_smem(smem_storage->q_smem);
...
#pragma unroll
        for (uint32_t j = 0; j < 2; ++j) {
          // fetch lse from global memory
          uint32_t qo_idx, r, qo_head_idx;
          gqa_group_size.divmod(qo_packed_idx_base + mma_q * 16 + lane_idx / 4 + 8 * j, qo_idx, r);
          qo_head_idx = kv_head_idx * gqa_group_size + r;
          if (qo_idx < q_len) {
            lse[mma_q][j] =
                params.final_lse[(q_indptr + qo_idx) * num_kv_heads * gqa_group_size + qo_head_idx];
          } else {
            lse[mma_q][j] = -math::inf;
          }
        }
      }
      ...
      LOOP_SPLIT_MASK(
          ...
          kv_tile_idx + 1 > NUM_STAGES, {
            ...
            logits_to_scores_and_reduction<KTraits>(
                ...
                kv_start + (kv_tile_idx * NUM_WARPS_KV + get_warp_idx_kv<KTraits>(tid.z)) *
                               NUM_MMA_KV * 16,
                q_len, kv_len, gqa_group_size, s_frag, lse, scores_smem, tid, kv_head_idx);
          });
...
template <uint32_t CTA_TILE_Q_1, uint32_t CTA_TILE_Q_2, uint32_t HEAD_DIM_QK, uint32_t HEAD_DIM_VO,
          MaskMode MASK_MODE, typename AttentionVariant, typename Params>
cudaError_t BatchAttentionScorePersistent(const Params params_1, const Params params_2,
                                          const uint32_t num_blks_x, const uint32_t num_blks_y,
                                          const cudaStream_t stream) {
  using DTypeQ = typename Params::DTypeQ;
  using DTypeKV = typename Params::DTypeKV;
